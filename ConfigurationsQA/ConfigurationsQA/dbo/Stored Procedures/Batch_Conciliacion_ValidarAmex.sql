 ...
 BEGIN TRANSACTION;
	...
	INNER JOIN Configurations.dbo.Detalle_Archivo da
	     ...
	     WHERE ac.flag_procesado = 0
	       AND ac.descripcion = @cod_medio_pago;
...
BEGIN
	...
	SELECT 
	 ...
	 ,(CASE WHEN SUBSTRING(detalles, 45, 1) = '4' THEN CAST(CONVERT(XML, '<amex><t>' + REPLACE(detalles, ',', '</t><t>') + '</t></amex>') AS XML).value('(/amex/t)[9]', 'INT')
	   ELSE NULL END)
	 ,(CASE WHEN SUBSTRING(detalles, 45, 1) IN ('4','5') THEN CAST(CONVERT(XML, '<amex><t>' + REPLACE(detalles, ',', '</t><t>') + '</t></amex>') AS XML).value('(/amex/t)[2]', 'NVARCHAR(10)')
	   ELSE NULL END)
