...
DECLARE @id_proceso INT = 11;
DECLARE @nombre_sp VARCHAR(200);
DECLARE @registros_afectados INT = NULL;
...
SET NOCOUNT ON;
	...
	-- parametros para log
	SET @detalle = 'Rango de fechas: @fecha_desde = ' + CAST(@fecha_desde AS VARCHAR) + ' @fecha_hasta = ' + CAST(@fecha_hasta AS VARCHAR);

	-- Actualizar Log     
	EXEC Configurations.dbo.Batch_Log_Actualizar_Proceso @id_log_proceso,
		@fecha_desde,
		@fecha_hasta,
		@registros_afectados,
		@Usuario;
	...
	SELECT tx.id_cuenta AS id_cuenta,
		tx.fecha_procesada AS fecha_procesada,
		--cantidades
		SUM(CASE 
				WHEN tx.tipo_tx = 'CREDITO'
					THEN 1
				ELSE 0
				END) AS cant_tx_dia_TC,
		SUM(CASE 
				WHEN tx.tipo_tx = 'DEBITO'
					THEN 1
				ELSE 0
				END) AS cant_tx_dia_TD,
		SUM(CASE 
				WHEN tx.tipo_tx = 'CUPON'
					THEN 1
				ELSE 0
				END) AS cant_tx_dia_cupon,
		SUM(CASE 
				WHEN tx.tipo_tx = 'CUPON VENCIDO'
					THEN 1
				ELSE 0
				END) AS cant_tx_dia_cupon_vencido,
		SUM(CASE 
				WHEN tx.tipo_tx = 'CASHOUT'
					THEN 1
				ELSE 0
				END) AS cant_tx_dia_cashOut,
		SUM(CASE 
				WHEN tx.tipo_tx = 'CREDITO'
					AND tx.canal = 'mPOS'
					THEN 1
				ELSE 0
				END) AS cant_tx_dia_TC_mPos,
		SUM(CASE 
				WHEN tx.tipo_tx = 'DEBITO'
					AND tx.canal = 'mPOS'
					THEN 1
				ELSE 0
				END) AS cant_tx_dia_TD_mPos,
		-- montos
		SUM(CASE 
				WHEN tx.tipo_tx = 'CREDITO'
					THEN ISNULL(tx.importe, 0)
				ELSE 0
				END) AS monto_tx_dia_TC,
		SUM(CASE 
				WHEN tx.tipo_tx = 'DEBITO'
					THEN ISNULL(tx.importe, 0)
				ELSE 0
				END) AS monto_tx_dia_TD,
		SUM(CASE 
				WHEN tx.tipo_tx = 'CUPON'
					THEN ISNULL(tx.importe, 0)
				ELSE 0
				END) AS monto_tx_dia_cupon,
		SUM(CASE 
				WHEN tx.tipo_tx = 'CUPON VENCIDO'
					THEN ISNULL(tx.importe, 0)
				ELSE 0
				END) AS monto_tx_dia_cupon_vencido,
		SUM(CASE 
				WHEN tx.tipo_tx = 'CASHOUT'
					THEN ISNULL(tx.importe, 0)
				ELSE 0
				END) AS monto_tx_dia_cashOut,
		SUM(CASE 
				WHEN tx.tipo_tx = 'CREDITO'
					AND tx.canal = 'mPOS'
					THEN ISNULL(tx.importe, 0)
				ELSE 0
				END) AS monto_tx_dia_TC_mPos,
		SUM(CASE 
				WHEN tx.tipo_tx = 'DEBITO'
					AND tx.canal = 'mPOS'
					THEN ISNULL(tx.importe, 0)
				ELSE 0
				END) AS monto_tx_dia_TD_mPos
	FROM (
		-- TARJETA y EFECTIVO
		SELECT trn.LocationIdentification AS id_cuenta,
			...
			(
				CASE 
					WHEN LTRIM(RTRIM(tmp.codigo)) = 'EFECTIVO'
						THEN (
								CASE 
									WHEN (
											LTRIM(RTRIM(trn.CouponStatus)) = 'VENCIDO'
											OR LTRIM(RTRIM(trn.TransactionStatus)) = 'TX_VENCIDA'
											)
										THEN 'CUPON VENCIDO'
									ELSE 'CUPON'
									END
		...
		WHERE LTRIM(RTRIM(tmp.codigo)) IN (
				'CREDITO',
				'DEBITO',
				'EFECTIVO'
				)
		...
		WHERE rdo.cod_respuesta_interno = - 1
			AND rdo.fecha_alta BETWEEN @fecha_desde
				AND @fecha_hasta
		) tx
	GROUP BY tx.id_cuenta,
		tx.fecha_procesada
...
BEGIN TRY
	...
	BEGIN
		...
		USING #INFO AS src
			ON (
					trg.id_cuenta = src.id_cuenta
					AND trg.fecha_procesada = src.fecha_procesada
					)
		WHEN MATCHED
			THEN
				UPDATE
