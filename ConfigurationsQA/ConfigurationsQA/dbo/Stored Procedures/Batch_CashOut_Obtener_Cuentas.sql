...
CREATE PROCEDURE dbo.Batch_CashOut_Obtener_Cuentas(
      @registros_procesados INT OUTPUT, @err VARCHAR(255) OUTPUT
)              
AS                        
...
SET NOCOUNT ON;          
 ...
 BEGIN TRY 
	...
	--Verificamos que exista el registro en el dia a verificar
	IF NOT  EXISTS (	

	       SELECT TOP 1 *  FROM Configurations.dbo.Feriados  fro  WHERE  CAST(fro.fecha AS DATE) = CAST(GETDATE() AS DATE)

	)
	THROW 51000,' Registro faltante Tabla Feriado ', 1;


	SELECT @Esferiado = fro.esFeriado
	FROM Configurations.dbo.Feriados fro 
	WHERE CAST(fro.fecha AS DATE) = CAST(GETDATE() AS DATE)
	AND fro.habilitado = 1 AND esFeriado = 0;   
		 
  --  THROW 51000,'error!!!!!!!' ,1;
    
	IF NOT  EXISTS (	

	       SELECT TOP 1 *  FROM Configurations.dbo.Parametro p  WHERE p.codigo = 'URL_WS_NOTIFICACION'

	)
	THROW 51000,' Parametro faltante URL_WS_NOTIFICACION', 1;
	...
	IF NOT  EXISTS (	
	     ...
	     SELECT TOP 1 *  FROM Configurations.dbo.Parametro p  WHERE p.codigo = 'URL_WS_CASHOUT'

	)
	THROW 51000,' Parametro faltante URL_WS_CASHOUT', 1;
	...
	IF NOT  EXISTS (	
	    ...
	    SELECT TOP 1 *  FROM Configurations.dbo.Parametro p  WHERE p.codigo = 'URL_WS_LOGIN_BATCH'

	)
	THROW 51000,' Parametro faltante URL_WS_LOGIN_BATCH', 1;
