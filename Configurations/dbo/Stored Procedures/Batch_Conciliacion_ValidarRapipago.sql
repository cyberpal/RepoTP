...
BEGIN TRY
    ...
    SELECT TOP 1
       @fecha_archivo = SUBSTRING(detalles,7,2)+SUBSTRING(detalles,5,2)+SUBSTRING(detalles,3,2)	
    FROM Configurations.dbo.Detalle_Archivo
	...
	ELSE IF(CHARINDEX(@fecha_archivo,@archivo_entrada) = 0)
      BEGIN
    	SET @motivo_rechazo = 'La fecha de proceso informada en el archivo no concuerda con la definido en el nombre del mismo.';
      END
	ELSE
	  BEGIN
