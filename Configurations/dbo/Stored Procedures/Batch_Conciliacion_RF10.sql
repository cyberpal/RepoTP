...
SET NOCOUNT ON;
	...
	INSERT INTO Configurations.dbo.Notificaciones_tmp (
		...
		SELECT 
		    cln.id_conciliacion,
		    '{"idNotificacion":5,"idCuenta":"'+CAST(tc.LocationIdentification AS VARCHAR(10))+'","values":{"codigoBanco":null,"nombreVendedor":"'+CONCAT(cta.denominacion2,' ',cta.denominacion1)+'","concepto":"'+tc.SaleConcept+'","montoBruto":"'+CAST(tc.Amount AS VARCHAR(12))+'","cliente":"'+tc.credentialholdername+'","mailCliente":"'+tc.CredentialEmailAddress+'","motivoRechazo":null,"TransactionId":"'+cln.id_transaccion+'"}}',
			@url
