		...
		BEGIN
			BEGIN TRY
				...
				BEGIN
                  ...
                  --------- Obtener valor de @id_motivo_ajuste_positivo

					--Cuenta del Buyer de la TRX Original
					  DECLARE @BuyerAccountTXOriginal int;
					  SELECT @BuyerAccountTXOriginal = trn.BuyerAccountIdentification  
					  FROM Transactions.dbo.Transactions trn 
					  WHERE trn.Id = @OriginalOperationId;  
  
					  --PRINT CONCAT('@BuyerAccountTXOriginal = ',@BuyerAccountTXOriginal)
  
					--Provincia del Buyer de la TRX Original
					  DECLARE @IdProvinciaCompradorTXOriginal INT;
					  SELECT @IdProvinciaCompradorTXOriginal = dca.id_provincia  
					   FROM Configurations.dbo.Domicilio_Cuenta dca  
							inner join Configurations.dbo.Tipo tpo  
								on tpo.id_tipo = dca.id_tipo_domicilio  
					   WHERE tpo.codigo = 'DOM_LEGAL'  
						 AND dca.id_cuenta = @BuyerAccountTXOriginal  
					     and dca.flag_vigente = 1;  
					 
					   --PRINT CONCAT('@IdProvinciaCompradorTXOriginal = ',@IdProvinciaCompradorTXOriginal)
					 
				  SELECT @id_motivo_ajuste_positivo = ipo.id_motivo_ajuste_positivo
				  ...
				  WHERE pva.id_provincia = @IdProvinciaCompradorTXOriginal
				    AND ipo.codigo <> 'IVA'
				   
				   --PRINT CONCAT('@id_motivo_ajuste_positivo = ' , @id_motivo_ajuste_positivo)
