...
CREATE TRIGGER [dbo].[CREATE_API_KEY] ON [Configurations].[dbo].[Cuenta] FOR INSERT
AS 
BEGIN
...
CREATE TRIGGER [Insert_History_Estado_Cuenta] ON dbo.Cuenta
	...
	BEGIN
		...
		BEGIN
			...
			SELECT id_cuenta,id_tipo_cuenta,denominacion1,denominacion2,id_tipo_identificacion,numero_identificacion,numero_CUIT,sexo,id_nacionalidad,fecha_nacimiento,id_canal,id_estado_cuenta,id_version_tyc,flag_envio_novedades,fecha_modificacion,usuario_modificacion,NULL,NULL,fecha_baja,usuario_baja,version,telefono_movil,telefono_fijo,flag_cambio_pendiente,flag_informado_a_facturacion,id_operador_celular,id_banco_adhesion,flag_factor_validacion FROM deleted;
		END
	END